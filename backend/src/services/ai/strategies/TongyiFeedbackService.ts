 ...
 * 通义大模型批改服务实现类
 */
export class TongyiFeedbackService extends BaseFeedbackService {
  // 通义API接口地址
  private static readonly API_URL = 'https://dashscope.aliyuncs.com/compatible-mode/v1';
  
  // 通义模型名称（用于提示词模板查找）
  private static readonly MODEL_NAME = 'tongyi';
  ...
  private initializeClient() {
    try {
      if (!this.config.apiKey) {
        console.warn('[通义] API密钥未设置，客户端未初始化');
        return;
      }
      
      this.openaiClient = new OpenAI({
        apiKey: this.config.apiKey,
        baseURL: TongyiFeedbackService.API_URL
      });
      
      console.log('[通义] 客户端初始化成功');
    } catch (error: any) {
      console.error('[通义] 客户端初始化失败:', error.message);
  ...
  getCapabilities(): {
    ...
    return {
      maxInputTokens: 8000,  // 通义大模型支持的最大输入token
  ...
  private async generateSingleStepFeedback(request: IELTSFeedbackRequest): Promise<IELTSFeedbackResult> {
    ...
    // 使用配置中的模型名称
    const modelName = this.config.model_name || 'qwen-max';
    ...
    const promptConfig = await PromptManager.getPrompt(
      TongyiFeedbackService.MODEL_NAME, // 这里只是用于模板查找
    ...
    const responseContent = await this.callTongyiAPI(
  ...
  private async callTongyiAPI(
    ...
    const chatMessages = [
      ...
      { role: 'user', content: userMessage } as ChatCompletionUserMessageParam
    ];
    ...
    // 检查API密钥和客户端初始化
    if (!this.config.apiKey || !this.openaiClient) {
      errorMessage = `${this.getServiceName()}API密钥未设置或客户端未初始化`;
      status = 'error';
    ...
    try {
      console.log(`[${this.getServiceName()}] [请求ID: ${requestId}] [类型: ${requestType}] [段落: ${paragraphInfo}] [项目ID: ${projectId}] [版本: ${versionNumber}] 调用API | 模型: ${this.config.model_name || 'qwen-max'}`);
      
      // 使用配置中的模型名称或默认使用qwen-max模型
      const response = await this.openaiClient.chat.completions.create({
        model: this.config.model_name || 'qwen-max',
  ...
  async getAnnotation(content: string, context?: {
    ...
    try {
      ...
      // 获取对应的提示词模板
      const promptConfig = await PromptManager.getPrompt(
        TongyiFeedbackService.MODEL_NAME,
        this.examType.toLowerCase(),
        taskType,
  ...
  async getExampleEssay(prompt: string, context?: {
    ...
    try {
      ...
      try {
        ...
        console.log(`[${this.getServiceName()}] [项目ID: ${projectId}] [版本: ${versionNumber}] 使用提示词 | 模型: ${modelName} | 使用模板: ${TongyiFeedbackService.MODEL_NAME} | 考试: ${examType} | 任务: ${taskType}`);
        ...
        if (examType.toLowerCase() === 'ielts') {
          ...
          try {
            ...
            // 渲染提示词
            const renderedPrompt = template.render(templateData);
            console.log(`[${this.getServiceName()}] [项目ID: ${projectId}] [版本: ${versionNumber}] 使用备选提示词 | 模型: ${modelName} | 使用模板: ${TongyiFeedbackService.MODEL_NAME} | 考试: ${examType} | 任务: ${taskType}`);
